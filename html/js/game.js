...
function preload() {
	...
	game.load.spritesheet('axe', 'img/axe-sheet.png', 64, 64);
	...
	if (key1.isDown)
	{
		if (key1.duration < 100)
		{
			if (direction == 1) 
			{
				...
				{
					attack = attacks.create(player.body.x + 25, player.body.y - 50, 'axe'); 	
					attack.scale.setTo(1.5, 1.5);
					attack.frame = 0;
				}
			}		
			...
			if (direction == 2)
			{
				...
				{
					attack = attacks.create(player.body.x - 75, player.body.y - 50, 'axe'); 	
					attack.scale.setTo(1.5, 1.5);
					attack.frame = 7;
				}	
			}	
